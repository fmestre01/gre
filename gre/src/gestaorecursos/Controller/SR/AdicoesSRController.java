...
import gestaorecursos.model.DAO.SRAdicoesDAO;
 ...
 * @author mgonc
 */
public class AdicoesSRController implements Initializable {
    ...
    private final SRAdicoesDAO sradicoesDao = new SRAdicoesDAO();
    ...
    public void handleBtnConfirmar() throws IOException, SQLException, Exception {
        ...
        boolean erro = false;
        try {
            a.setData_adicao(ConversorData.converter(inputData.getValue()));
        } catch (Exception e) {
        ...
        a.setHistorico_adicao(txtFieldHistorico.getText());
        a.setVlr_adicao(nf.parse(txtFieldValor.getText()).doubleValue());
        a.setPrograma(cbxPrograma.getValue());
        a.setUex(cbxUex.getValue());
        a.setAno(cbxAno.getValue());
        a.setTrimestre(cbxTrimestre.getValue());

        sradicoesDao.inserir(a);
    ...
    public AdicoesSR getAdicoesSR() {
        return adicoesSR;
    }

    public void setAdicoesSR(AdicoesSR adicoesSR) {
        this.adicoesSR = adicoesSR;
    }
